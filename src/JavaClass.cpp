...
JavaClass::JavaClass(InjectionClient *client, std::string className)
  ...
  this->clazz =
      client->getENV()->FindClass(Mapper::getNameOfClass(className).c_str());
}

jmethodID JavaClass::getMethod(std::string name) {
...
jfieldID JavaClass::getField(std::string name) {
...
jfieldID JavaClass::getFieldRAW(std::string name, std::string description,
                                bool isMethodStatic) {
...
jmethodID JavaClass::getMethodRAW(std::string name, std::string description,
                                  bool isMethodStatic) {
  return isMethodStatic ? client->getENV()->GetStaticMethodID(
                              clazz, name.c_str(), description.c_str())
...
template <typename T>
jobject JavaClass::getObject(jmethodID method, T values...) {
  return client->getENV()->CallStaticObjectMethod(clazz, method, values);
}

jobject JavaClass::getObject(jmethodID method) {
