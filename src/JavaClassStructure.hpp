...
public:
  ...
  JavaClassStructure *getClassAsContext(std::string name) {
    this->context = name;

    return this;
  }

  JavaClassStructure *addField(std::string name, std::string obfName,
                               std::string description, bool isMethodStatic) {
  ...
  JavaClassStructure *addMethod(std::string name, std::string obfName,
                                std::string description, bool isMethodStatic) {
    Logger::logInfo("OBF NAME \"" + obfName +
                    "\" (method) is being mapped to: " + name);
    JavaClassMethod mth = JavaClassMethod(obfName, description, isMethodStatic);
    nameToClassLayout[context].addMethod(std::make_pair(name, mth));

    return this;
  }

  JavaClassStructure *addClass(std::string className, std::string obfName) {
  ...
  JavaClassLayout getClassLayout(std::string className) {
